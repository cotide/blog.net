...
(function ($) {    
       ...
       var settings = {  
            ...
            if($checkElement.length>0) { 
                ...
                $from.submit(function() { 
                    ...
                    $checkElement.each(function() {
                        ...
                        if(settings.showType=="alter") {
                           if($isPass) { 
                               $isPass=startValidator(obj,$isPass); 
                           } 
                        } else {
                           $isPass=startValidator(obj,$isPass);  
                        }
               ...
               $checkElement.each(function(i){ 
                   ...
                   if(objDefaultMsg!=undefined) {
                         showMessage(obj[0].name, "Default", objDefaultMsg);
                         if(settings.showType=="alter") {
                   ...
                   if(objFocusMsg!=undefined) {
                       obj.bind("focus", function() {
                           ...
                           showMessage(obj[0].name, "Focus", objFocusMsg);
                       });
                   }; 
                   if(settings.showType!="alter") { 
                       obj.bind(settings.validatorEvent, function() {
                            return startValidator(obj);
                       });
                   }  
        ...
        //////////////////////// 方法 //////////////////////// 
         // 开始验证
         var startValidator = function(obj,isPass) { 
            // 验证
            ...
            if ( isEmptyAttr != "true") {
                ...
                validatorResult = checkIsEmpty(obj, emptyMsg); 
            } 
             // 如果有值，继续进行其他校验
            if(obj.val()!="") {
                isEmptyAttr = "false";
            ...
            if (getElemType(obj) == "INPUT") {
                ...
                if (isEmptyAttr != "true"  && validatorResult) { 
                    ...
                    if(minValue|| maxValue) { 
                       validatorResult = checkLenRange(obj, rangeMsg, minValue, maxValue); 
                    }
                }
                // 进行正则验证
                if (isEmptyAttr != "true" && validatorResult) {
                    ...
                    if (regex) {
                        ...
                        validatorResult = checkRegex(obj, regex, regexMsg);
                    }
                } 
                ...
                if(isEmptyAttr != "true" && validatorResult) { 
                    ...
                    if(compareElem) { 
                      ...
                      validatorResult =  checkCompareElem(obj, compareElem, compareMsg);
                    }
                }
                ...
                if(isEmptyAttr != "true" && validatorResult) { 
                    ...
                    if(ajaxUrl) {
                      // validatorResult = undefined;
                       showMessage(obj[0].name, "Loadding");
                        var value =$.trim( obj.val()); 
                           $.ajax({
                                ...
                                success: function (data) {
                                    if(data) {
                                        if(data.IsRight) {  
                                             showMessage(obj[0].name, "Right"); 
                                        } else { 
                                             showMessage(obj[0].name,"Error", data.Msg); 
                                        }
                                    } else {
                                        showMessage(obj[0].name, "Error","访问超时，请稍后重试");  
                                    }
                                },
            ...
            // elem  ------ 元素对象 
            // msg -------- 提示消息
            var checkIsEmpty = function(elem,msg) {
                 ...
                 if(obj.val()=="") {
                     if(settings.showType=="show") {
                         showMessage(obj[0].name, "Error", msg);
                     } else {
                         alert(msg); 
                     }
                     return false;
            ...
            // 值对比
            var checkCompareElem = function(elem,compareElem,msg) {
                 var obj = elem;
                 if(!(elem instanceof jQuery)){
                     obj = $(elem);
                 } 
           ...
           // 长度验证
            var checkLenRange = function(elem,msg,min,max) {
                 var obj = elem;
                 if(!(elem instanceof jQuery)){
                     obj = $(elem);
                 } 
            ...
            // 正则验证
            var checkRegex = function(elem,regex,msg) { 
                    var obj = elem;
                    if(!(elem instanceof jQuery)){
                        obj = $(elem);
                    } 
            ...
            // 提示内容  
            var  showMessage  = function(elemId,msgType,msg)
            { 
                var tipObj = $("#"+elemId+settings.tipSuffix); 
                if(tipObj[0]) { 
                    // 错误提示
                    if(msgType=="Error") { 
                        tipObj.html("&nbsp;"); 
